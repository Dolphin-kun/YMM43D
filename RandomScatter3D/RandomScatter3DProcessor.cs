        ...
        : VideoEffect3DProcessorBase(effect, devices)
    {
        ...
        public override void Draw(in Render3DContext render, DrawContext3D item)
        {
            ...
            foreach (var copy in Copies(time))
            {
                ...
                var world = effect.FacesCamera
                    ? scale * Matrix4x4.CreateRotationZ(-float.DegreesToRadians(copy.Angles.Z)) * facing
                      * Matrix4x4.CreateTranslation(Vector3.Transform(position, item.World))
                    : scale * Rotation3D.ForObject(copy.Angles.X, copy.Angles.Y, copy.Angles.Z)
                      * Matrix4x4.CreateTranslation(position) * item.World;

                pipeline.Draw(
                    render.Context,
                    render.CreateConstants(world, item.Opacity, effect.IsUnlit, item.AlphaCutoff, gloss),
                    settings);
            }
        }
        ...
        protected override WorldBounds GetLocalBounds(in FrameContext itemTime)
        {
            ...
            var reach = TryGetSize(out var size, out _) ? MathF.Max(size.X, size.Y) / 2f : 0f;
            var half = WorldScale.ToWorldPosition(range.X / 2f + reach, range.Y / 2f + reach, range.Z / 2f + reach);
            var extent = Vector3.Abs(half);
